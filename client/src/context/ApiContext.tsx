...
export const ApiProvider: React.FC<ApiProviderProps> = ({ children }) => {
    ...
    useEffect(() => {
        ...
        const MAX_FAILURES = 2; // Reduced to 2 to detect offline state quicker
        ...
        const checkApiAvailability = async () => {
            try {
                ...
                // Check for CORS errors which indicate API is offline
                ...
                // CORS errors mean API is definitely offline, so mark it immediately
                if (isCorsError) {
                    setIsApiOnline(false);
                    logger.error(`API health check failed with CORS/Network error:`, error);
                    return;
                }

                // For other errors, only set API as offline after consecutive failures
